...
def _fetch_from_alpha_vantage(ticker: str, api_key: str) -> dict[str, Any]:
...
@tool
def get_market_data(ticker: str) -> TickerAnalysis | dict[str, Any]:
    """
    ...
    if not api_key:
        if settings.allow_mock_data:
            logger.warning("Alpha Vantage key absent — returning illustrative data for %s", ticker)
            return _MOCK_DATA.get(ticker, _build_placeholder(ticker))
        return {"error": "Alpha Vantage is not configured"}

    try:
        raw = _fetch_from_alpha_vantage(ticker, api_key)
        if "Note" in raw:
            logger.warning("Alpha Vantage rate limit reached for %s", ticker)
            if settings.allow_mock_data:
                return _MOCK_DATA.get(ticker, _build_placeholder(ticker))
        ...
        if settings.allow_mock_data:
            return _MOCK_DATA.get(ticker, _build_placeholder(ticker))
        return {"error": "Market data provider request failed"}
